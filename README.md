...
### Conclusion
The shell feels like a regular shell, especially using the functions provided by the readline library which store the history of non-empty commands and offer auto-completion using a specified key (tab) and after having implemented ```cd``` alongside ```ls```.

### Remaining Extensions:
- [x] tab twice to show possible files
